...
from transformers import BertModel, XLMRobertaModel, AlbertModel, RobertaModel
...
class SpERTBaseClass():
    """ Span-based model to jointly extract entities and relations """

    VERSION = '1.1'

    def __init__(self, config, cls_token: int, relation_types: int, entity_types: int,
                 size_embedding: int, prop_drop: float, freeze_transformer: bool, max_pairs: int = 100):
        super(SpERTBaseClass, self).__init__(config)

        # Для каждого типа модели нужна своя переменная в self для модели,
        # название переменной должно соответствовать полю base_model_prefix
        # из BertPreTrainedModel, XLMRobertaPretrainedModel, AlbertPreTrainedModel
        self.spert_model_type = None
        if config.__class__.__name__ == "BertConfig":
            # BERT model
            self.bert = BertModel(config)
            self.spert_model_type = "bert"
        elif config.__class__.__name__ == "XLMRobertaConfig":
            # XLM-RoBERTa
            self.roberta = XLMRobertaModel(config)
            self.spert_model_type = "roberta"
        elif config.__class__.__name__ == "AlbertConfig":
            # Albert
            self.albert = AlbertModel(config)
            self.spert_model_type = "albert"
        elif config.__class__.__name__ == "RobertaConfig":
            # Roberta
        ...
        if freeze_transformer:
            ...
            if self.spert_model_type == "roberta":
                for param in self.roberta.parameters():
                    param.requires_grad = False
            elif self.spert_model_type == "bert":
                for param in self.bert.parameters():
                    param.requires_grad = False
            elif self.spert_model_type == "albert":
    ...
    def _forward_train(self, encodings: torch.tensor, context_masks: torch.tensor, entity_masks: torch.tensor,
        ...
        if self.spert_model_type == "roberta":
            ...
            h = self.albert(input_ids=encodings, attention_mask=context_masks)['last_hidden_state']
...
class SpERT(SpERTBaseClass, BertPreTrainedModel):
    def __init__(self, config: BertConfig, cls_token: int, relation_types: int, entity_types: int,
                 size_embedding: int, prop_drop: float, freeze_transformer: bool, max_pairs: int = 100):
        super(SpERT, self).__init__(config, cls_token, relation_types, entity_types,
                 size_embedding, prop_drop, freeze_transformer, max_pairs)

class SpERT_XLMRoberta(SpERTBaseClass, RobertaPreTrainedModel):
    def __init__(self, config: XLMRobertaConfig, cls_token: int, relation_types: int, entity_types: int,
                 size_embedding: int, prop_drop: float, freeze_transformer: bool, max_pairs: int = 100):
        super(SpERT_XLMRoberta, self).__init__(config, cls_token, relation_types, entity_types,
                 size_embedding, prop_drop, freeze_transformer, max_pairs)


class SpERT_Albert(SpERTBaseClass, AlbertPreTrainedModel):
    def __init__(self, config: AlbertConfig, cls_token: int, relation_types: int, entity_types: int,
                 size_embedding: int, prop_drop: float, freeze_transformer: bool, max_pairs: int = 100):
        super(SpERT_Albert, self).__init__(config, cls_token, relation_types, entity_types,
                 size_embedding, prop_drop, freeze_transformer, max_pairs)
...
_MODELS = {
    'spert': SpERT,
    'spert-xlmroberta': SpERT_XLMRoberta,
    'spert-albert': SpERT_Albert,
    'spert-roberta': SpERT_XLMRoberta,
}
