...
from transformers import AdamW, BertConfig, XLMRobertaConfig, AlbertConfig, RobertaConfig
from transformers import BertTokenizer, XLMRobertaTokenizer, AlbertTokenizer, RobertaTokenizer
...
class SpERTTrainer(BaseTrainer):
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        if self._args.model_type=="spert":
            ...
            self._tokenizer = XLMRobertaTokenizer.from_pretrained(args.tokenizer_path,
                                                            ...
                                                            unk_token='<unk>',
                                                            pad_token='<s>')
        elif self._args.model_type == "spert-albert":
            self._tokenizer = AlbertTokenizer.from_pretrained(args.tokenizer_path,
                                                              do_lower_case=args.lowercase,
                                                              cache_dir=args.cache_path)
        elif self._args.model_type == "spert-roberta":
    ...
    def _load_model(self):
        ...
        if self._args.model_type=="spert":
            ...
            config = AlbertConfig.from_pretrained(self._args.model_path, cache_dir=self._args.cache_path,
                                                  pad_token_id=0, bos_token_id=1)
        elif self._args.model_type=="spert-roberta":
        ...
        model = model_class.from_pretrained(self._args.model_path,
                                            ...
                                            cls_token=self._tokenizer.convert_tokens_to_ids(str(self._tokenizer._cls_token)),
                                            relation_types= 9 - 1,
                                            entity_types= 19,
    ...
    def _predict(self, model: torch.nn.Module, dataset: Dataset, input_reader: BaseInputReader, tmpFilePath):
        # create data loader
        dataset.switch_mode(Dataset.EVAL_MODE)
        data_loader = DataLoader(dataset, batch_size=self._args.eval_batch_size, shuffle=False, drop_last=False,
                                 num_workers=self._args.sampling_processes, collate_fn=sampling.collate_fn_padding)
